...
export function ActiveWorkout() {
    ...
    useEffect(() => {
        fetch("/api/member/active-workout")
            .then(res => res.json())
            .then(d => {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-20 space-y-4">
            ...
            <p className="text-[10px] font-black italic text-slate-500 uppercase tracking-widest">FETCHING_PROTOCOL...</p>
        </div>
    );
    ...
    if (!data) return (
        <Card className="bg-slate-950/20 border-white/5 p-12 text-center">
            ...
            <h3 className="text-xl font-black italic uppercase tracking-tighter text-slate-400">NO_ACTIVE_PROTOCOLS</h3>
            <p className="text-[10px] text-slate-500 mt-2 font-black italic uppercase tracking-widest">CONTACT_YOUR_TRAINER_TO_INITIALIZE_ASSIGNMENT</p>
        </Card>
    );
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <Badge variant="outline" className="bg-primary/10 text-primary border-none text-[8px] font-black italic tracking-[0.2em] mb-2 uppercase">
                            DAY_{data.currentDay.dayNumber}_SEQUENCE
                        </Badge>
                    ...
                    <div className="text-right">
                        <span className="text-[9px] font-black italic text-slate-500 uppercase tracking-widest">EST_DURATION</span>
            ...
            <div className="space-y-4">
                {totalExercises === 0 ? (
                    <Card className="bg-slate-950/20 border-white/5 border-dashed p-12 text-center text-slate-500">
                        ...
                        <h3 className="text-xl font-black italic uppercase tracking-widest text-slate-400">ACTIVE_RECOVERY</h3>
                        <p className="text-[10px] uppercase font-bold tracking-widest mt-2">NO_REQUIRED_SEQUENCES_TODAY</p>
                    </Card>
                ) : (
                                ...
                                {ex.notes && (
                                    <div className="px-5 pb-4 border-t border-white/5 pt-3">
                                        <p className="text-[10px] text-slate-500 font-medium italic italic">
                                            <span className="text-primary/50 font-black mr-2 uppercase tracking-widest">INTEL:</span>
                                            {ex.notes}
                                        </p>
                                    </div>
                                )}
