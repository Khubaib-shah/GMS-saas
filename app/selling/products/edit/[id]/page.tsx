        ...
        try {
            ...
            if (res.ok && data) {
                ...
                setFormData({
                    ...
                    nutritionalInfo: {
                        servingSize: p.nutritionalInfo?.servingSize || "",
                        calories: p.nutritionalInfo?.calories?.toString() || "",
                        protein: p.nutritionalInfo?.protein?.toString() || "",
                        carbs: p.nutritionalInfo?.carbs?.toString() || "",
                        fat: p.nutritionalInfo?.fat?.toString() || ""
                    }
                });

                if (p.thumbnail) setThumbnailAsset(p.thumbnail);
                if (p.images) setGalleryAssets(p.images);
            } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast.error(err.error || "Failed to update product", { id: "product-update" });
            }
        } catch (err: any) {
            toast.error(err.message || "An error occurred", { id: "product-update" });
        } finally {
            setSaving(false);
        }
    };
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center p-40 space-y-4">
                ...
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Hydrating Product Data...</p>
            </div>
        );
    }
    ...
    return (
        <div className="mx-auto md:p-8 space-y-6 md:space-y-8 animate-in slide-in-from-bottom-4 duration-700 pb-20">
            <DashboardHeader
                ...
                description={`Updating: ${formData.name}`}
            >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Same form structure as NewProduct */}
                <div className="lg:col-span-2 space-y-6 md:space-y-8">
                    <Card className="glass-premium bg-slate-950/20 border-white/5 p-6 md:p-10 space-y-6 md:space-y-8 rounded-[24px] md:rounded-[32px] relative overflow-hidden">
                        ...
                        <div className="space-y-6 relative">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                                <InputField
                                    ...
                                    className="h-12 md:h-14 rounded-xl md:rounded-2xl bg-black/40 border-white/10 focus:border-primary/50 transition-all text-slate-400 font-mono text-[9px] md:text-[10px] px-4 md:px-6"
                                />
                                <Field>
                                    ...
                                    >
                                        <SelectTrigger className="w-full h-12 md:h-14 rounded-xl md:rounded-2xl bg-black/40 border border-white/10 focus:border-primary/50 transition-all font-black text-[9px] md:text-[10px] tracking-widest px-4 md:px-6 outline-none text-white cursor-pointer">
