...
import { Shield, ArrowRight, CheckCircle, Eye, EyeOff, AlertCircle, Loader2 } from "lucide-react";
import { signIn } from "next-auth/react";
import { toast } from "sonner";
import { ConfirmModal } from "@/components/ui/confirm-modal";

type LoginMode = "LOGIN" | "FORGOT_EMAIL" | "FORGOT_OTP" | "FORGOT_NEW_PASSWORD";
    ...
    <div className="w-full max-w-md">
        ...
        {mode !== "LOGIN" && (
          <>
            ...
            <p className="text-[14px] text-white/40">
              ...
              {mode === "FORGOT_NEW_PASSWORD" && "Create a new strong password."}
            </p>
          </>
        )}
      </div>
      ...
      {mode === "FORGOT_EMAIL" && (
        <form onSubmit={handleRequestOtp} className="flex flex-col gap-5">
          <InputField
            ...
            required
          />
          <Button type="submit" disabled={loading} className="btn-nav-secondary w-full">
            {loading ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <span>Get a Recovery Code</span>}
          </Button>
          <button type="button" onClick={() => setMode("LOGIN")} className="text-xs text-white/40 hover:text-white mt-2">
            Back to login
          </button>
        </form>
      )}
      ...
      {mode === "FORGOT_OTP" && (
        <form onSubmit={handleVerifyOtp} className="flex flex-col gap-5">
          ...
          />
          <Button type="submit" disabled={loading} className="btn-nav-secondary w-full">
            {loading ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <span>Verify Code</span>}
          </Button>
          <div className="flex flex-col items-center gap-2 mt-2">
            {countdown > 0 ? (
              <p className="text-xs text-white/40">Code expires in <span className="text-primary font-bold">{countdown}s</span></p>
            ) : (
              <button type="button" onClick={() => handleRequestOtp()} disabled={loading} className="text-xs text-primary hover:text-white transition-colors">
                Resend Code
              </button>
            )}
            <button type="button" onClick={() => setMode("LOGIN")} className="text-xs text-white/40 hover:text-white mt-2">
