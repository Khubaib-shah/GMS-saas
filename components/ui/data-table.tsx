        ...
        <div className="space-y-4 w-full">
            {(searchKey || filter || table.getAllColumns().some(col => col.getCanHide())) && (
                <div className="flex flex-col md:flex-row items-center justify-between gap-2 md:gap-4">
                    {searchKey && (
                        <div className="flex items-center flex-1 w-full md:max-w-sm relative group">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-2 w-full md:w-auto ml-auto">
                        {filter && <div className="w-full md:w-auto">{filter}</div>}
                        ...
                        <DropdownMenu >
                            <DropdownMenuTrigger asChild>
                                <Button
                                    variant="outline"
                                    size="sm"
                                    className="w-full md:w-auto h-10 flex bg-white/5 border-white/10 text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-black rounded-xl gap-2 px-4"
                                >
                                    ...
                                    .map((column) => {
                                        return (
                                            <DropdownMenuCheckboxItem
                                                key={column.id}
                                                className={cn(
                                                    "capitalize text-[10px] font-bold text-slate-400 focus:bg-primary focus:text-black",
                                ...
                                {headerGroup.headers.map((header) => {
                                    return (
                                        <TableHead
                                            key={header.id}
                                            className={cn(
                                                "h-12 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 px-1 md:px-6",
                        ...
                        {isLoading ? (
                            Array.from({ length: 5 }).map((_, i) => (
                                <TableRow key={i} className="border-white/5">
                                    {columns.map((_, index) => (
                                        <TableCell key={index} className="px-1 md:px-6 py-2 md:py-4">
                            ...
                            table.getRowModel().rows.map((row) => (
                                ...
                                >
                                    {row.getVisibleCells().map((cell) => (
                                        <TableCell key={cell.id} className={cn("px-1 md:px-6 py-2 md:py-4 text-[11px] font-semibold md:font-bold tracking-wide text-slate-300", (cell.column.columnDef.meta as any)?.className)}>
                            ...
                            <TableRow>
                                <TableCell colSpan={columns.length} className="h-24 text-center text-[10px] font-black uppercase tracking-widest text-slate-500">
                                    No results found.
                                </TableCell>
                            </TableRow>
                        )}
