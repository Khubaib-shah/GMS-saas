...
export function Navbar() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-slate-950/80 backdrop-blur-md border-b border-white/[0.06] shadow-lg shadow-black/20" : "bg-transparent"}`}
    >
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className="nav-link-reveal text-[13px] md:text-[18px]"
            >
              <div className="nav-text">
                {l.label.split(" ").map((w, i) => (
                  <span key={i}>{w}</span>
                ))}
              </div>
        ...
        <div className="md:hidden border-t border-white/[0.06] bg-slate-950/95 backdrop-blur-lg px-6 py-8 flex flex-col gap-6 shadow-2xl animate-in slide-in-from-top-4 duration-200">
          <div className="flex flex-col gap-4">
            {navLinks.map((l) => (
              <Link
                ...
                className="text-[16px] font-medium text-white/70 hover:text-white transition-colors py-1"
              >
                {l.label}
              </Link>
            ))}
          ...
          <div className="flex flex-col gap-3 pt-6 border-t border-white/[0.06]">
            <Link href="/login" onClick={() => setIsOpen(false)}>
              <Button
                variant="outline"
                className="w-full h-11 border-white/10 text-white bg-white/5 hover:bg-white/[0.08] hover:text-white rounded-xl"
              >
                Sign in
              </Button>
            </Link>
